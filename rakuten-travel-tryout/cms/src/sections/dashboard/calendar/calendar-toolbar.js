...
import { Button, IconButton, Stack, SvgIcon, TextField, Typography, useMediaQuery } from '@mui/material';
...
export const CalendarToolbar = props => {
  const { date, onAddClick, onDateNext, onDatePrev, onDateToday, onViewChange, view, ...other } = props;
  ...
  const availableViewOptions = useMemo(() => {
    return mdUp ? viewOptions : viewOptions.filter(option => ['timeGridDay', 'listWeek'].includes(option.value));
  }, [mdUp]);
