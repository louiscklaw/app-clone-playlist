...
import { Box, Card, Checkbox, Chip, Divider, FormControlLabel, Input, Stack, SvgIcon, Typography } from '@mui/material';
...
export const JobListSearch = props => {
  ...
  const levelValues = useMemo(() => chips.filter(chip => chip.field === 'level').map(chip => chip.value), [chips]);
