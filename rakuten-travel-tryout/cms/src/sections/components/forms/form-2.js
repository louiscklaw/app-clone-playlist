...
import { Box, Checkbox, Chip, Divider, FormControlLabel, Input, Stack, SvgIcon } from '@mui/material';
...
export const Form2 = () => {
  ...
  const statusValues = useMemo(() => chips.filter(chip => chip.field === 'status').map(chip => chip.value), [chips]);

  const stockValues = useMemo(() => chips.filter(chip => chip.field === 'inStock').map(chip => chip.value), [chips]);
