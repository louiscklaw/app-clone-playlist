...
async function HotelInfo() {
  ...
  for (var i = 0; i < room_list.length; i++) {
    const created = await prisma.roomSetup.upsert({
      where: { id: i + 1 },
      update: {},
      create: { name: `room_${i}` },
    });
  ...
  const result_category = await prisma.planSetup.findMany({
  ...
  //             description: '',
  //             images: 'https://trvis.r10s.com/d/strg/ctrl/26/21edcacd90fefea7251afed2e11ecabca9e5ef68.47.1.26.2.jpg',
  //             points: '日式床組 x 3 單人床 x 4,129m²,1–7 位住客,禁菸',
  //           },
  //         ],
  //       },
  //       plans: {
